...
with customer_orders as (
    ...
    select
        ...
        c.primary_state as customer_state,
    ...
    join {{ ref('dim_customers') }} c on o.customer_unique_id = c.customer_unique_id
    where o.is_delivered
    group by c.customer_unique_id, c.primary_state
),
...
with_rankings as (
    select
        *,
        ...
        ntile(10) over (order by lifetime_revenue desc, customer_unique_id) as revenue_decile,

        -- Quartile bucket
        ntile(4) over (order by lifetime_revenue desc, customer_unique_id) as revenue_quartile,

        -- Percentile (0.0 to 1.0)
        round(
            percent_rank() over (order by lifetime_revenue)::numeric, 4
